...
import {
  IconoOk, IconoGarantia, IconoTrofeo, IconoReloj,
  IconoUbicacion, IconoEstrella, IconoVolver,
} from "./Iconos";
...
const SOLUCIONADOR = {
  ...
  miembro: "Marzo 2023",
  descripcion: "Electricista matriculado con más de 8 años de experiencia en instalaciones residenciales y comerciales. Trabajo con materiales de primera calidad y garantizo todos mis trabajos.",
  garantia: "30 días",
...
const RESEÑAS = [
  { id: 1, usuario: "Martín G.", inicial: "M", estrellas: 5, fecha: "hace 2 semanas", texto: "Excelente trabajo. Llegó puntual, resolvió el problema rápido y dejó todo limpio. Lo recomiendo.",    trabajo: "Cambio de tablero eléctrico" },
  { id: 2, usuario: "Laura P.",  inicial: "L", estrellas: 5, fecha: "hace 1 mes",     texto: "Muy profesional. Explicó todo lo que iba haciendo y el precio fue justo.",                               trabajo: "Instalación de aire acondicionado" },
  { id: 3, usuario: "Carlos R.", inicial: "C", estrellas: 4, fecha: "hace 2 meses",   texto: "Buen trabajo, tardó un poco más de lo esperado pero el resultado fue muy bueno.",                        trabajo: "Reparación de circuito" },
];
    ...
    <div className={styles.pantalla}>
      ...
      <div className={styles.heroBloque}>
        <div className={styles.heroAcciones}>
          <button className={styles.btnVolver} onClick={() => {
            if (desdeParam === "busqueda") navigate("/busqueda?paso=3");
            else if (desdeParam === "seguimiento") navigate(`/seguimiento${solIdParam ? `?solId=${solIdParam}` : ""}`);
            else if (desdeParam === "presupuestos" && trabajoIdBack) navigate(`/presupuestos?trabajoId=${trabajoIdBack}`);
            else navigate(-1);
          }}>
            <IconoVolver size={20} />
          </button>
          <button className={styles.btnCompartir} onClick={() => mostrarToast("Compartir perfil")}>
        ...
        <div className={styles.heroInfo}>
          ...
          <p className={styles.heroOficio}>{SOLUCIONADOR_ACTIVO.oficio} · Nivel {SOLUCIONADOR_ACTIVO.nivel}</p>
          <div className={styles.heroStats}>
            <div className={styles.heroStat}>
              <span className={styles.heroStatNum}>{SOLUCIONADOR_ACTIVO.calificacion || SOLUCIONADOR_ACTIVO.reputacion}</span>
              <span className={styles.heroStatLabel}><IconoEstrella size={11} /> Reputación</span>
            </div>
            <div className={styles.heroStatDivider}></div>
            <div className={styles.heroStat}>
              <span className={styles.heroStatNum}>{SOLUCIONADOR_ACTIVO.trabajos || SOLUCIONADOR_ACTIVO.totalTrabajos}</span>
              <span className={styles.heroStatLabel}><IconoTrofeo size={11} /> Trabajos</span>
            </div>
            <div className={styles.heroStatDivider}></div>
            <div className={styles.heroStat}>
              <span className={styles.heroStatNum}>{SOLUCIONADOR_ACTIVO.tiempoRespuesta}</span>
              <span className={styles.heroStatLabel}><IconoReloj size={11} /> Respuesta</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className={styles.tabs}>
        {[
          ...
          <button key={tab.id} type="button"
            ...
            <span className={styles.tabInner}>{tab.icono}{tab.label}</span>
          </button>
        ))}
      </div>
          ...
          <div className={styles.tabContenido}>
            ...
            <section className={styles.seccion}>
              <h2 className={styles.seccionTitulo}>
                <IconoOk size={16} className={styles.seccionIconoVerde} /> Verificaciones
              </h2>
              <div className={styles.certificacionesLista}>
                {SOLUCIONADOR_ACTIVO.certificaciones.map((cert, i) => (
                  <div key={i} className={styles.certificacionItem}>{cert}</div>
                ))}
              </div>
            </section>
            ...
            <section className={styles.seccion}>
              <h2 className={styles.seccionTitulo}>
                <IconoTrofeo size={16} className={styles.seccionIconoRojo} /> Servicios que ofrece
              </h2>

              {/* Principales */}
              {SOLUCIONADOR_ACTIVO.oficios.filter(o => o.tipo === "principal").length > 0 && (
                <div className={styles.oficiosGrupo}>
                  <p className={styles.oficiosGrupoLabel}>⭐ Principal</p>
                  <div className={styles.oficiosLista}>
                    {SOLUCIONADOR_ACTIVO.oficios.filter(o => o.tipo === "principal").map(oficio => (
                      <div key={oficio.nombre} className={`${styles.oficioCard} ${styles.oficioCardPrincipal}`}>
                        <span className={styles.oficioIcono}>{oficio.icono}</span>
                        <span className={styles.oficioNombre}>{oficio.nombre}</span>
                        <span className={styles.oficioStar}>⭐</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {SOLUCIONADOR_ACTIVO.oficios.filter(o => o.tipo === "secundario").length > 0 && (
                <div className={styles.oficiosGrupo}>
                  <p className={styles.oficiosGrupoLabel}>🔵 Secundario</p>
                  <div className={styles.oficiosLista}>
            ...
            <section className={styles.garantiaCard}>
              ...
              <div className={styles.garantiaInfo}>
                <p className={styles.garantiaTitulo}>
                  <IconoGarantia size={14} className={styles.garantiaIconoInline} /> Garantía de trabajo
                </p>
                <p className={styles.garantiaValor}>{SOLUCIONADOR_ACTIVO.garantia}</p>
                <p className={styles.garantiaDesc}>Si algo falla en ese período, vuelve sin costo adicional</p>
              </div>
            </section>
            ...
            <section className={styles.precioCard}>
              <div className={styles.precioInfo}>
                ...
                <p className={styles.precioDesc}>El precio final se define en el presupuesto</p>
              </div>
              <div className={styles.precioDistancia}>
                <p className={styles.precioDistanciaNum}>{SOLUCIONADOR_ACTIVO.distancia}</p>
          ...
          <div className={styles.tabContenido}>
            ...
            <div className={styles.fotosGrid}>
              {FOTOS_TRABAJOS.map((foto, i) => (
                <div key={i} className={styles.fotoCard} onClick={() => mostrarToast("Ver foto " + (i + 1))}>
            ...
            <p className={styles.fotosNota}>📸 Las fotos son de trabajos reales verificados por TeePee</p>
          </div>
        )}
        ...
        {tabActiva === "disponibilidad" && (
          <div className={styles.tabContenido}>
            <p className={styles.disponibilidadDesc}>
              Días y horarios en que {SOLUCIONADOR_ACTIVO.nombre.split(" ")[0]} suele estar disponible. Confirmá por chat antes de agendar.
            </p>

            {/* Encabezado turnos */}
            <div className={styles.dispEncabezado}>
              {TURNOS.map(t => (
                <div key={t.id} className={styles.dispEncabezadoLabel}>{t.label}</div>
              ))}
            </div>

            {/* Grilla */}
            <div className={styles.dispGrilla}>
              {DIAS_SEMANA.map(fila => {
                const activo = SOLUCIONADOR_ACTIVO.disponibilidad.includes(fila.key);
                return (
                  <div key={fila.key} className={styles.dispFila}>
                    <span className={`${styles.dispDiaLabel} ${!activo ? styles.dispDiaInactivo : ""}`}>
                      {fila.dia}
                    </span>
                    {TURNOS.map(turno => {
                      ...
                      return (
                        <div key={turno.id}
                          className={`${styles.dispTurnoCell} ${disponible ? styles.dispTurnoCellActivo : styles.dispTurnoCellVacio}`}>
          ...
          <div className={styles.tabContenido}>
            <div className={styles.reputacionResumen}>
              ...
              <div className={styles.reputacionDetalle}>
                ...
                <p className={styles.reputacionTotal}>{SOLUCIONADOR_ACTIVO.totalReseñas} reseñas verificadas</p>
              </div>
            </div>
                ...
                <div key={reseña.id} className={styles.reseñaCard}>
                  <div className={styles.reseñaHeader}>
                    ...
                    <div className={styles.reseñaHeaderInfo}>
                      <span className={styles.reseñaUsuario}>{reseña.usuario}</span>
                      <span className={styles.reseñaFecha}>{reseña.fecha}</span>
                    </div>
                    <div className={styles.reseñaEstrellas}>{"⭐".repeat(reseña.estrellas)}</div>
                  </div>
                  <p className={styles.reseñaTexto}>{reseña.texto}</p>
                  <div className={styles.reseñaTrabajo}>🔧 {reseña.trabajo}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className={styles.footer}>
        <div className={styles.footerPrecio}>
          ...
          <span className={styles.footerPrecioMonto}>{SOLUCIONADOR_ACTIVO.precio}</span>
        </div>
        <button type="button" className={styles.btnContactar} onClick={() => setModalContacto(true)}>
          Contactar →
        </button>
      </div>
      ...
      {/* Modal contacto */}
      {modalContacto && (
        <div className={styles.modalOverlay} onClick={e => { if (e.target === e.currentTarget) setModalContacto(false); }}>
          <div className={styles.modal}>
            <div className={styles.modalHandle}></div>
            <div className={styles.modalIcono}>⚡</div>
            <h2 className={styles.modalTitulo}>Contactar a {SOLUCIONADOR_ACTIVO.nombre}</h2>
            <p className={styles.modalDesc}>Al contactarlo vas a poder chatear, recibir su presupuesto y coordinar el trabajo.</p>
            <div className={styles.modalOpciones}>
              <button type="button" className={styles.modalOpcion}
                onClick={() => { setModalContacto(false); navigate(`/chat?solId=${solIdParam || 1}`); }}>
                <span>💬</span>
                <div>
                  <span className={styles.modalOpcionTitulo}>Enviar mensaje</span>
                  <span className={styles.modalOpcionDesc}>Chateá y pedí presupuesto</span>
                </div>
                <span>›</span>
              </button>
              <button type="button" className={`${styles.modalOpcion} ${styles.modalOpcionUrgente}`}
                onClick={() => { setModalContacto(false); navigate(`/chat?solId=${solIdParam || 1}`); }}>
                <span>🚨</span>
                <div>
                  <span className={styles.modalOpcionTitulo}>Solicitud urgente</span>
                  <span className={styles.modalOpcionDesc}>Necesito atención inmediata</span>
                </div>
                <span>›</span>
              </button>
            </div>
            <button type="button" className={styles.modalCancelar} onClick={() => setModalContacto(false)}>
              Cancelar
            </button>
          </div>
        </div>
