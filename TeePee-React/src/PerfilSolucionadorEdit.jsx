      ...
      <main className={styles.contenido}>
        ...
        {tab === 'profesional' && (
          <section className={styles.seccion}>
            ...
            <div className={styles.campoBloque}>
              ...
              {mostrarExtras && (
                <div style={{
                  ...
                  <p style={{ width: '100%', fontSize: 11, color: 'var(--tp-marron-suave)', margin: '0 0 6px', fontFamily: 'var(--fuente)', fontWeight: 600 }}>
                    ...
                    return (
                      <button
                        ...
                        style={{
                          ...
                          border: esPpal ? '2px solid var(--tp-rojo)' : esSec ? '2px solid #2A7D5A' : '1px solid rgba(61,31,31,0.15)',
                          background: esPpal ? 'var(--tp-rojo)' : esSec ? '#2A7D5A' : 'var(--tp-crema)',
                  ...
                  {/* Campo Otro oficio */}
                  <div style={{ width: '100%', marginTop: 8, paddingTop: 8, borderTop: '1px dashed rgba(61,31,31,0.12)' }}>
                    <p style={{ fontSize: 11, color: 'var(--tp-marron-suave)', margin: '0 0 6px', fontFamily: 'var(--fuente)', fontWeight: 600 }}>Otro oficio no listado:</p>
                    <div style={{ display: 'flex', gap: 8 }}>
                      <input
                        ...
                        style={{ flex: 1, padding: '7px 10px', borderRadius: 8, border: '1px solid rgba(61,31,31,0.15)', background: 'var(--tp-crema)', fontFamily: 'var(--fuente)', fontSize: 13, color: 'var(--tp-marron)', outline: 'none' }}
                      />
                      <select
                        value={datos.otroOficioTipo || 'secundario'}
                        onChange={e => set('otroOficioTipo', e.target.value)}
                        style={{ padding: '7px 8px', borderRadius: 8, border: '1px solid rgba(61,31,31,0.15)', background: 'var(--tp-crema)', fontFamily: 'var(--fuente)', fontSize: 12, color: 'var(--tp-marron)', cursor: 'pointer' }}
                      >
            ...
            <div className={styles.campoBloque}>
              ...
              <p className={styles.campoSublabel}>
                Títulos, certificados de oficio, constancias de trabajo, habilitaciones, etc. Podés adjuntar todos los que quieras.
              </p>
              ...
              {(datos.certificados || []).length > 0 && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 10 }}>
                  {(datos.certificados || []).map((cert, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', borderRadius: 'var(--r-md)', background: 'rgba(42,125,90,0.08)', border: '1px solid rgba(42,125,90,0.15)' }}>
                      <span style={{ fontSize: 18 }}>{cert.tipo === 'pdf' ? '📄' : '🖼️'}</span>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--tp-marron)', margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{cert.nombre}</p>
                        <p style={{ fontSize: 11, color: 'var(--tp-marron-suave)', margin: 0 }}>{cert.label}</p>
                      </div>
                      <button type="button"
                        onClick={() => set('certificados', (datos.certificados || []).filter((_, j) => j !== i))}
                        style={{ border: 'none', background: 'none', cursor: 'pointer', color: 'var(--tp-rojo)', fontSize: 16, padding: 4 }}>
                        ✕
                      </button>
                    </div>
                  ))}
              ...
              {/* Botón agregar */}
              <label style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, padding: '14px 12px', borderRadius: 'var(--r-md)', border: '1.5px dashed rgba(61,31,31,0.20)', background: 'none', cursor: 'pointer', fontFamily: 'var(--fuente)' }}>
                <span style={{ fontSize: 22 }}>📎</span>
                <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--tp-marron)' }}>Adjuntar documento / imagen</span>
                <span style={{ fontSize: 11, color: 'var(--tp-marron-suave)' }}>JPG, PNG, PDF — sin límite de cantidad</span>
        ...
        {tab === 'cobros' && (
          <section className={styles.seccion}>
            ...
            {/* Cuenta 1 */}
            <div className={styles.campoBloque} style={{ background: 'rgba(61,31,31,0.03)', borderRadius: 'var(--r-md)', padding: 12, border: '1px solid rgba(61,31,31,0.08)', marginBottom: 12 }}>
              <label className={styles.campoLabel} style={{ marginBottom: 8, display: 'block' }}>
                Cuenta 1 <span className={styles.req}>*</span>
              </label>
              <label className={styles.campoLabel}>CBU / CVU</label>
              <input type="text" className={styles.campoInput}
                placeholder="22 dígitos" value={datos.cbu} maxLength={22}
                onChange={e => set('cbu', e.target.value.replace(/\D/g, ''))} />
              <span className={styles.campoHint}>{datos.cbu.length}/22 dígitos</span>
              <label className={styles.campoLabel} style={{ marginTop: 8 }}>Alias (opcional)</label>
              <input type="text" className={styles.campoInput}
                placeholder="tu.alias.mp" value={datos.aliasCbu}
                onChange={e => set('aliasCbu', e.target.value)} />
              {datos.cbu.length === 22 && <div className={styles.cbuOk}>✅ CBU válido</div>}
            </div>

            {/* Cuenta 2 */}
            <div className={styles.campoBloque} style={{ background: 'rgba(61,31,31,0.03)', borderRadius: 'var(--r-md)', padding: 12, border: '1px solid rgba(61,31,31,0.08)', marginBottom: 16 }}>
              <label className={styles.campoLabel} style={{ marginBottom: 8, display: 'block' }}>
                Cuenta 2 <span style={{ fontSize: 10, color: 'var(--tp-marron-suave)', fontWeight: 400 }}>(opcional)</span>
              </label>
              <label className={styles.campoLabel}>CBU / CVU</label>
              <input type="text" className={styles.campoInput}
                placeholder="22 dígitos" value={datos.cbu2 || ''} maxLength={22}
                onChange={e => set('cbu2', e.target.value.replace(/\D/g, ''))} />
              <span className={styles.campoHint}>{(datos.cbu2 || '').length}/22 dígitos</span>
              <label className={styles.campoLabel} style={{ marginTop: 8 }}>Alias (opcional)</label>
              <input type="text" className={styles.campoInput}
                placeholder="tu.alias2.mp" value={datos.aliasCbu2 || ''}
                onChange={e => set('aliasCbu2', e.target.value)} />
            </div>

            {/* Datos de facturación */}
            <p style={{ fontSize: 12, fontWeight: 700, color: 'var(--tp-marron)', margin: '0 0 10px', fontFamily: 'var(--fuente)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
              Datos de facturación
            </p>
            ...
            <div className={styles.campoBloque}>
              ...
              <input type="text" className={styles.campoInput}
                placeholder="XX-XXXXXXXX-X" value={datos.cuit || ''}
                maxLength={13}
        ...
        {tab === 'docs' && (
          <section className={styles.seccion}>
            ...
            <div className={styles.campoBloque} style={{ marginTop: 8 }}>
              <label className={styles.campoLabel}>
                Boleta de servicio (agua, luz, gas) <span className={styles.req}>*</span>
              </label>
              <p className={styles.campoSublabel}>
              ...
              {datos.fotoBoleta ? (
                <div className={styles.dniPreview}>
                  ...
                  <button type="button" className={styles.dniCambiar}
                  ...
                  <input type="file" accept="image/*,application/pdf" className={styles.fotoInput}
                    onChange={e => {
                      ...
                      if (file) {
                        ...
                        reader.onload = ev => set('fotoBoleta', ev.target.result);
