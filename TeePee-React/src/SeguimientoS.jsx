...
export default function SeguimientoS() {
  ...
  const t = {
    ...trabajoData,
    avanceEnviado: trabajoData.avanceReportado,
    cliente: {
      nombre:  usuData.nombre,
      inicial: usuData.inicial,
      color:   usuData.color,
    },
    etapasPago: trabajoData.etapasPago.map(e => ({
      ...e,
      estado: e.estado === "habilitado" ? "pendiente" :
              e.estado === "pagado"     ? "cobrado"   : e.estado,
    })),
  };
  ...
  const [avancesAjuste, setAvancesAjuste]   = useState({}); // avance por id de ajuste
  ...
  // Monto que corresponde al avance actual reportado (incremento sobre el aprobado)
  const montoAvanceActual = Math.round(((avanceActual - avanceAprobado) / 100) * t.monto);
  ...
  return (
    <div style={{ background: "var(--tp-crema)", minHeight: "100vh", fontFamily: "var(--fuente)" }}>
      ...
      <header style={{ position: "sticky", top: 0, zIndex: 100, background: "var(--tp-crema)", borderBottom: "1px solid rgba(61,31,31,0.08)", padding: "14px 16px", display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <button type="button"
          onClick={() => navigate(`/chat-s?usuarioId=${trabajoData.usuarioId}&nombre=${encodeURIComponent(t.cliente.nombre)}&inicial=${t.cliente.inicial}&desde=seguimiento-s&trabajoId=${trabajoData.id}`)}
      ...
      <div style={{ padding: "14px 16px 90px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 14px", background: "var(--tp-crema-clara)", borderRadius: "var(--r-lg)", marginBottom: 12, border: "1px solid rgba(61,31,31,0.08)" }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <p style={{ fontSize: 12, color: "var(--tp-marron-suave)", margin: 0 }}>Cliente</p>
          </div>
          <button type="button"
            onClick={() => navigate(`/acuerdo-digital?desde=seguimiento-s&trabajoId=${trabajoData.id}&modo=firmado`)}
        ...
        <div style={{ background: "var(--tp-crema-clara)", borderRadius: "var(--r-lg)", padding: 16, marginBottom: 12, border: "1px solid rgba(61,31,31,0.08)" }}>
          ...
          <div style={{ position: "relative", height: 14, borderRadius: 7, background: "rgba(61,31,31,0.08)", marginBottom: 8, overflow: "hidden" }}>
            <div style={{ position: "absolute", left: 0, top: 0, bottom: 0, borderRadius: 7, background: "var(--verde)", width: `${avanceAprobado}%`, transition: "width 0.6s ease" }} />
            {avanceActual > avanceAprobado && (
              <div style={{ position: "absolute", left: `${avanceAprobado}%`, top: 0, bottom: 0, borderRadius: "0 7px 7px 0", width: `${avanceActual - avanceAprobado}%`, background: "repeating-linear-gradient(45deg, rgba(245,200,66,0.6), rgba(245,200,66,0.6) 4px, rgba(245,200,66,0.25) 4px, rgba(245,200,66,0.25) 8px)" }} />
            )}
          </div>
          ...
          <div style={{ marginTop: 14 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
              ...
              <div style={{ textAlign: "right" }}>
                <p style={{ fontSize: 15, fontWeight: 900, color: "var(--tp-rojo)", margin: 0 }}>{avanceActual}%</p>
                {avanceActual > avanceAprobado && (
                  <p style={{ fontSize: 11, fontWeight: 700, color: "var(--tp-marron-suave)", margin: 0 }}>
                    Certifica ${montoAvanceActual.toLocaleString("es-AR")}
                  </p>
                )}
              </div>
            </div>
            ...
            {avanceActual > avanceAprobado && (
              <div style={{ marginTop: 10, padding: "10px 12px", borderRadius: "var(--r-md)", background: "rgba(184,64,48,0.06)", border: "1px solid rgba(184,64,48,0.15)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div>
                  <p style={{ fontSize: 10, fontWeight: 700, color: "var(--tp-marron-suave)", textTransform: "uppercase", letterSpacing: "0.5px", margin: "0 0 2px", fontFamily: "var(--fuente)" }}>Certificado de avance</p>
                  <p style={{ fontSize: 11, color: "var(--tp-marron-suave)", margin: 0, fontFamily: "var(--fuente)" }}>
                    {avanceAprobado}% → {avanceActual}% · +{avanceActual - avanceAprobado}% del total
                  </p>
                </div>
                <p style={{ fontSize: 20, fontWeight: 900, color: "var(--tp-rojo)", margin: 0, fontFamily: "var(--fuente)" }}>
                  ${montoAvanceActual.toLocaleString("es-AR")}
                </p>
              </div>
            )}
          </div>
        ...
        {/* Historial de ajustes propuestos */}
        {ajustes.length > 0 && (
          <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            {ajustes.map((aj, i) => (
              <div key={aj.id} style={{ padding: "12px 14px", borderRadius: "var(--r-md)", background: "var(--tp-crema-clara)", border: `1px solid ${aj.estado === "aprobado" ? "rgba(42,125,90,0.25)" : "rgba(245,200,66,0.40)"}` }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
                  <div>
                    <p style={{ fontSize: 15, fontWeight: 900, color: "var(--tp-marron)", margin: "0 0 2px", fontFamily: "var(--fuente)" }}>
                      +${aj.monto.toLocaleString("es-AR")}
                    </p>
                    <p style={{ fontSize: 11, color: "var(--tp-marron-suave)", margin: 0, fontFamily: "var(--fuente)" }}>
                      Neto: ${aj.neto.toLocaleString("es-AR")} · Comisión: ${aj.comision.toLocaleString("es-AR")}
                    </p>
                  </div>
                ...
                <p style={{ fontSize: 12, color: "var(--tp-marron-suave)", margin: 0, fontFamily: "var(--fuente)", lineHeight: 1.5 }}>{aj.descripcion}</p>
                <p style={{ fontSize: 10, color: "var(--tp-marron-suave)", margin: "4px 0 0", fontFamily: "var(--fuente)" }}>Propuesto a las {aj.fecha}</p>
              </div>
            ))}
            {/* Monto total actualizado */}
            <div style={{ display: "flex", justifyContent: "space-between", padding: "10px 14px", borderRadius: "var(--r-md)", background: "var(--tp-marron)" }}>
              <span style={{ fontSize: 13, color: "rgba(240,234,214,0.70)", fontFamily: "var(--fuente)" }}>Monto total actualizado</span>
              <span style={{ fontSize: 15, fontWeight: 900, color: "var(--tp-crema)", fontFamily: "var(--fuente)" }}>
                ${(t.monto + ajustes.filter(a => a.estado === "aprobado").reduce((s, a) => s + a.monto, 0)).toLocaleString("es-AR")}
                {ajustes.some(a => a.estado === "pendiente") && (
                  <span style={{ fontSize: 11, color: "rgba(245,200,66,0.8)", marginLeft: 6 }}>
                    +${ajustes.filter(a => a.estado === "pendiente").reduce((s, a) => s + a.monto, 0).toLocaleString("es-AR")} pendiente
                  </span>
                )}
              </span>
            </div>
      ...
      {modalAjuste && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(61,31,31,0.55)", zIndex: 200, display: "flex", alignItems: "flex-end" }}>
          <div style={{ background: "var(--tp-crema)", borderRadius: "20px 20px 0 0", padding: 24, width: "100%", maxHeight: "85vh", overflowY: "auto" }}>
            <p style={{ fontSize: 16, fontWeight: 800, color: "var(--tp-marron)", margin: "0 0 4px", fontFamily: "var(--fuente)" }}>Proponer ajuste de monto</p>
            <p style={{ fontSize: 12, color: "var(--tp-marron-suave)", margin: "0 0 14px", fontFamily: "var(--fuente)", lineHeight: 1.6 }}>
              El cliente recibirá una notificación y deberá aceptar antes de continuar.
            </p>
            ...
            <div style={{ background: "var(--tp-crema-clara)", borderRadius: "var(--r-md)", padding: 14, border: "1px solid rgba(61,31,31,0.08)", marginBottom: 10 }}>
              <p style={{ fontSize: 10, fontWeight: 700, color: "var(--tp-marron-suave)", textTransform: "uppercase", margin: "0 0 6px", fontFamily: "var(--fuente)" }}>Incremento de monto</p>
              <div style={{ display: "flex", alignItems: "baseline", gap: 6 }}>
                <span style={{ fontSize: 20, color: "var(--tp-marron-suave)" }}>$</span>
                <input type="number" placeholder="0" value={ajusteMonto}
              ...
              {/* Desglose neto/comisión — solo visible para el solucionador */}
              {ajusteMonto > 0 && (
                <div style={{ marginTop: 10, paddingTop: 8, borderTop: "1px dashed rgba(61,31,31,0.12)" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}>
                    <span style={{ fontSize: 12, color: "var(--tp-marron-suave)", fontFamily: "var(--fuente)" }}>Tu cobro neto (94%)</span>
                    <span style={{ fontSize: 13, fontWeight: 700, color: "var(--tp-marron)", fontFamily: "var(--fuente)" }}>${Math.round(ajusteMonto * 0.94).toLocaleString("es-AR")}</span>
                  </div>
                  <div style={{ display: "flex", justifyContent: "space-between" }}>
                    <span style={{ fontSize: 12, color: "var(--tp-marron-suave)", fontFamily: "var(--fuente)" }}>Comisión TeePee (6%)</span>
                    <span style={{ fontSize: 12, color: "var(--tp-marron-suave)", fontFamily: "var(--fuente)" }}>${Math.round(ajusteMonto * 0.06).toLocaleString("es-AR")}</span>
                  </div>
                </div>
              )}
            </div>
            ...
            <div style={{ background: "var(--tp-crema-clara)", borderRadius: "var(--r-md)", padding: 14, border: "1px solid rgba(61,31,31,0.08)", marginBottom: 14 }}>
              <p style={{ fontSize: 10, fontWeight: 700, color: "var(--tp-marron-suave)", textTransform: "uppercase", margin: "0 0 6px", fontFamily: "var(--fuente)" }}>Descripción detallada</p>
              <textarea placeholder="Describí en detalle los trabajos adicionales o materiales que justifican el ajuste..."
                value={ajusteDesc} onChange={e => setAjusteDesc(e.target.value)} rows={4}
            ...
            <button type="button"
              onClick={() => {
                if (!ajusteMonto || !ajusteDesc.trim()) { mostrarToast("⚠️ Completá el monto y la descripción"); return; }
                const nuevo = {
                  id: Date.now(),
                  monto: Number(ajusteMonto),
                  neto: Math.round(ajusteMonto * 0.94),
                  comision: Math.round(ajusteMonto * 0.06),
                  descripcion: ajusteDesc,
                  estado: "pendiente",
                  fecha: new Date().toLocaleTimeString("es-AR", { hour: "2-digit", minute: "2-digit" }),
                };
                setAjustes(prev => [...prev, nuevo]);
                setAjusteMonto("");
                setAjusteDesc("");
