...
const SOLICITUDES_HOME = [
  {
    id: 1, cliente: "Laura Pérez", inicial: "L", color: "#B84030",
  ...
  {
    id: 2, cliente: "Roberto Silva", inicial: "R", color: "#2A7D5A",
  ...
  {
    id: 3, cliente: "Diego Fernández", inicial: "D", color: "#8C6820",
    servicio: "Cambio de canilla y sifón cocina",
    descripcionDetallada: "La canilla de la cocina gotea constantemente y el sifón está roto. Quisiera reemplazarla por una monocomando.",
    direccion: "Junín 789, Posadas",
    distancia: "1.8 km", presupuestoEstimado: "$8.000 - $15.000",
    urgente: false, fotos: 0,
    disponibilidad: [
      { dia: "Vie", turnos: ["7-12"] },
...
const PRESUPUESTOS_MOCK = [
  {
    id: 1, cliente: "Martín García", inicial: "M", color: "#B84030",
    servicio: "Pérdida de agua en baño principal",
    estado: "por_enviar", fecha: "Hoy", monto: null,
    solicitud: {
      descripcionDetallada: "Hay una pérdida importante debajo de la pileta del baño principal. El piso ya está húmedo y hay manchas en el cielorraso del piso de abajo. Urgente antes de que empeore.",
      urgente: false,
      direccion: "Av. Mitre 1240, Piso 3, Dpto B — Posadas",
  ...
  {
    id: 2, cliente: "Laura Sánchez", inicial: "L", color: "#2A7D5A",
    servicio: "Instalación calefón nuevo",
    estado: "esperando", fecha: "Hace 2 hs",
    monto: 32000, etapas: "50/50",
    solicitud: {
      descripcionDetallada: "Tengo un calefón nuevo marca Orbis 13 litros que compré pero no tengo quién lo instale. El viejo se rompió hace una semana. También necesito que revisen la conexión de gas.",
      urgente: true,
      direccion: "San Lorenzo 456, Posadas",
  ...
  },
  {
    ...
    estado: "aceptado", fecha: "Ayer",
    monto: 8500, etapas: "total",
    solicitud: {
      descripcionDetallada: "La canilla de la cocina gotea constantemente. Es una canilla de dos llaves, vieja. Me gustaría reemplazarla por una monocomando.",
      urgente: false,
      direccion: "Junín 789, Posadas",
      distancia: "2.1 km",
  ...
  {
    id: 4, cliente: "Roberto Silva", inicial: "R", color: "#534AB7",
    servicio: "Reparación cañería externa",
    estado: "borrador", fecha: "Hace 3 días",
    monto: 45000, etapas: "30/40/30",
    solicitud: {
      descripcionDetallada: "Se rompió una cañería en el exterior de la casa, creo que es la de agua fría. Hay un charco permanente cerca de la medianera. La casa tiene 20 años.",
      urgente: false,
  ...
  return (
    <div style={{ fontFamily: "var(--fuente)" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
        <button onClick={paso === 2 ? () => setPaso(1) : onVolver}
          style={{ border: "none", background: "none", cursor: "pointer", color: "var(--tp-marron-suave)", display: "flex" }}>
          <IconoVolver size={20} />
        </button>
        <div style={{ flex: 1 }}>
          <p style={{ fontSize: 15, fontWeight: 800, color: "var(--tp-marron)", margin: 0 }}>
        ...
        <button type="button"
          onClick={() => navigate(`/chat-s?usuarioId=1&nombre=${encodeURIComponent(solicitud.cliente)}&inicial=${solicitud.inicial}`)}
          style={{ width: 34, height: 34, borderRadius: "50%", border: "1px solid rgba(61,31,31,0.15)", background: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: "var(--tp-marron-suave)" }}
      ...
      {sol && (
        <div style={{ ...sCard, marginBottom: 14, background: "var(--tp-crema-oscura)", border: "1px solid rgba(61,31,31,0.12)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
            <div style={{ width: 34, height: 34, borderRadius: "50%", background: solicitud.color, color: "white", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 800, flexShrink: 0 }}>
              {solicitud.inicial}
            </div>
            <div>
              <p style={{ fontSize: 13, fontWeight: 800, color: "var(--tp-marron)", margin: 0 }}>{solicitud.cliente}</p>
              <p style={{ fontSize: 11, color: "var(--tp-marron-suave)", margin: 0 }}>{solicitud.servicio}</p>
            </div>
            {sol.urgente && (
              <span style={{ marginLeft: "auto", fontSize: 10, fontWeight: 700, background: "var(--tp-rojo)", color: "white", padding: "2px 8px", borderRadius: 20 }}>🚨 URGENTE</span>
            )}
          </div>

          {/* Descripción detallada */}
          <p style={{ fontSize: 13, color: "var(--tp-marron)", margin: 0, lineHeight: 1.6, fontStyle: "italic" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 5, marginTop: 4 }}>
            <div style={{ display: "flex", alignItems: "flex-start", gap: 6 }}>
              ...
              <span style={{ fontSize: 12, color: "var(--tp-marron-suave)" }}>{sol.direccion} · {sol.distancia}</span>
            </div>
            <div style={{ display: "flex", alignItems: "flex-start", gap: 6 }}>
              <span style={{ fontSize: 13, flexShrink: 0 }}>💰</span>
              <span style={{ fontSize: 12, color: "var(--tp-marron-suave)" }}>Rango esperado: {sol.presupuestoEstimado}</span>
            </div>
            {sol.fotos > 0 && (
              <div>
                <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6 }}>
                  <span style={{ fontSize: 13 }}>📷</span>
                  <span style={{ fontSize: 12, color: "var(--tp-marron-suave)" }}>{sol.fotos} foto{sol.fotos > 1 ? "s" : ""} adjunta{sol.fotos > 1 ? "s" : ""}</span>
                </div>
                <div style={{ display: "flex", gap: 6 }}>
                  {Array.from({ length: sol.fotos }).map((_, i) => (
                    <div key={i} style={{
                      ...
                      fontSize: 22, border: "1px solid rgba(61,31,31,0.10)",
                    }}>📷</div>
                  ))}
                </div>
              </div>
          ...
          <div style={{ marginTop: 6 }}>
            <p style={{ ...sLabel, marginBottom: 6 }}>Disponibilidad del cliente</p>
            <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
              {sol.disponibilidad.map((fila, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <span style={{ fontSize: 12, fontWeight: 700, color: "var(--tp-marron)", width: 28, flexShrink: 0 }}>{fila.dia}</span>
                  <div style={{ display: "flex", gap: 4 }}>
                    {fila.turnos.map(t => (
                      <span key={t} style={{ fontSize: 10, padding: "2px 7px", borderRadius: 20, background: "rgba(61,31,31,0.10)", color: "var(--tp-marron)", fontWeight: 600 }}>{t}</span>
                    ))}
                  </div>
                </div>
              ))}
              {sol.horaPuntual && (
                <p style={{ fontSize: 11, color: "var(--tp-rojo)", margin: "2px 0 0", fontWeight: 600 }}>⏰ Hora puntual preferida: {sol.horaPuntual}</p>
              )}
            </div>
          </div>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              {[
                ...
                <button key={op.id} type="button"
                  ...
                  style={{
                    ...
                    background: tipo === op.id ? "var(--tp-marron)" : "rgba(61,31,31,0.06)",
                    color: tipo === op.id ? "var(--tp-crema)" : "var(--tp-marron)",
                  }}>
                  <div style={{ fontSize: 20, marginBottom: 4 }}>{op.emoji}</div>
                  <p style={{ fontSize: 12, fontWeight: 700, margin: "0 0 2px" }}>{op.titulo}</p>
                  <p style={{ fontSize: 10, opacity: 0.7, margin: 0 }}>{op.sub}</p>
                </button>
              ))}
            </div>
          ...
          <div style={sCard}>
            ...
            <textarea
              ...
              rows={4}
              style={{ width: "100%", fontSize: 13, color: "var(--tp-marron)", border: "none", background: "none", outline: "none", fontFamily: "var(--fuente)", resize: "none", lineHeight: 1.7 }}
            />
          </div>
      ...
      {paso === 2 && (
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          {/* Resumen del paso 1 */}
          <div style={{ ...sCard, flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
            <div>
              <p style={sLabel}>Monto acordado</p>
              <p style={{ fontSize: 22, fontWeight: 900, color: "var(--tp-marron)", margin: 0 }}>{montoFinal}</p>
            </div>
            <button onClick={() => setPaso(1)} style={{ fontSize: 12, color: "var(--tp-rojo)", border: "none", background: "none", cursor: "pointer", fontFamily: "var(--fuente)", fontWeight: 600 }}>
              Cambiar
            </button>
          </div>
              ...
              {etapasOpciones.map(op => (
                <button key={op.id} type="button" onClick={() => setEtapas(op.id)}
                  style={{
                    ...
                    background: etapas === op.id ? "var(--tp-marron)" : "rgba(61,31,31,0.05)",
                  }}>
                  <div style={{ width: 20, height: 20, borderRadius: "50%",
                    border: `2px solid ${etapas === op.id ? "var(--tp-crema)" : "rgba(61,31,31,0.25)"}`,
                    display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                    {etapas === op.id && <div style={{ width: 10, height: 10, borderRadius: "50%", background: "var(--tp-crema)" }} />}
                  </div>
                  <div>
                    <p style={{ fontSize: 13, fontWeight: 700, margin: 0, color: etapas === op.id ? "var(--tp-crema)" : "var(--tp-marron)" }}>{op.label}</p>
                    <p style={{ fontSize: 11, margin: 0, color: etapas === op.id ? "rgba(240,234,214,0.7)" : "var(--tp-marron-suave)" }}>{op.sub}</p>
                  </div>
                </button>
              ))}
            </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            <div style={sCard}>
              ...
              <select value={garantia} onChange={e => setGarantia(e.target.value)}
                style={{ fontSize: 14, fontWeight: 700, color: "var(--tp-marron)", border: "none", background: "none", fontFamily: "var(--fuente)", width: "100%" }}>
            ...
            <div style={sCard}>
              ...
              <div style={{ display: "flex", gap: 6 }}>
                {[true, false].map(v => (
                  <button key={String(v)} type="button" onClick={() => setMateriales(v)}
                    style={{ flex: 1, padding: "6px 4px", borderRadius: 8, cursor: "pointer", fontSize: 11, fontWeight: 600, border: "none", fontFamily: "var(--fuente)",
                      background: materiales === v ? "var(--tp-marron)" : "rgba(61,31,31,0.06)",
                      color: materiales === v ? "var(--tp-crema)" : "var(--tp-marron-suave)" }}>
                    {v ? "Incluidos" : "Sin mat."}
                  </button>
                ))}
              </div>
          ...
          {/* Botones de envío */}
          <button type="button" onClick={enviarAlChat}
            style={{ width: "100%", padding: 16, borderRadius: "var(--r-md)", background: "var(--tp-rojo)", color: "var(--tp-crema)", border: "none", cursor: "pointer", fontFamily: "var(--fuente)", fontSize: 15, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", gap: 8, opacity: enviandoChat ? 0.7 : 1 }}>
            <MessageCircle size={16} /> Enviar al chat del cliente
          </button>

          <button type="button" onClick={onEnviar}
            style={{ width: "100%", padding: 14, borderRadius: "var(--r-md)", background: "none", color: "var(--tp-marron)", border: "1.5px solid rgba(61,31,31,0.20)", cursor: "pointer", fontFamily: "var(--fuente)", fontSize: 14, fontWeight: 600, display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
      ...
      {/* Modal visita previa */}
      {modalVisita && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(61,31,31,0.6)", zIndex: 200, display: "flex", alignItems: "flex-end" }}>
          <div style={{ background: "var(--tp-crema)", borderRadius: "20px 20px 0 0", padding: 24, width: "100%" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
              <p style={{ fontSize: 16, fontWeight: 800, color: "var(--tp-marron)", margin: 0, fontFamily: "var(--fuente)" }}>🔍 Visita previa</p>
              <button onClick={() => setModalVisita(false)} style={{ border: "none", background: "none", cursor: "pointer" }}><X size={20} /></button>
            </div>
            <p style={{ fontSize: 13, color: "var(--tp-marron-suave)", marginBottom: 16, fontFamily: "var(--fuente)", lineHeight: 1.6 }}>
              Podés cobrar un costo de visita para evaluar el trabajo. Si el cliente acepta el presupuesto final, ese monto se descuenta del total.
            </p>
            <div style={sCard}>
              <p style={sLabel}>Costo de visita</p>
              <div style={{ display: "flex", alignItems: "baseline", gap: 6 }}>
                <span style={{ fontSize: 20, color: "var(--tp-marron-suave)" }}>$</span>
                <input type="number" placeholder="0  (dejar en 0 si es gratis)"
                  value={costoVisita} onChange={e => setCostoVisita(e.target.value)}
                  style={{ flex: 1, fontSize: 24, fontWeight: 800, color: "var(--tp-marron)", border: "none", background: "none", outline: "none", fontFamily: "var(--fuente)" }} />
              </div>
            </div>
            <button type="button" onClick={() => setModalVisita(false)}
              style={{ width: "100%", padding: 14, borderRadius: "var(--r-md)", background: "var(--tp-marron)", color: "var(--tp-crema)", border: "none", cursor: "pointer", fontFamily: "var(--fuente)", fontSize: 14, fontWeight: 700, marginTop: 14 }}>
              Confirmar
            </button>
          </div>
        </div>
...
const sCard = {
  ...
  padding: 14, border: "1px solid rgba(61,31,31,0.08)",
  display: "flex", flexDirection: "column", gap: 8,
};
const sLabel = {
  fontSize: 11, fontWeight: 700, color: "var(--tp-marron-suave)",
  textTransform: "uppercase", letterSpacing: "0.5px", margin: 0,
  fontFamily: "var(--fuente)", display: "flex", alignItems: "center",
};
...
export default function PresupuestosS() {
  ...
  const solicitudDesdeHome = solicitudIdParam
    ? (() => {
        ...
        if (!s) return null;
        return {
          cliente: s.cliente, inicial: s.inicial, color: s.color,
          servicio: s.servicio, monto: null,
          solicitud: {
            descripcionDetallada: s.descripcionDetallada,
            direccion: s.direccion, distancia: s.distancia,
            presupuestoEstimado: s.presupuestoEstimado,
            urgente: s.urgente, fotos: s.fotos,
            disponibilidad: s.disponibilidad, horaPuntual: s.horaPuntual,
          },
        };
      })()
    : null;
  ...
  const presupuestosFiltrados = filtro === "todos"
    ? PRESUPUESTOS_MOCK
    : filtro === "pendientes"
    ? PRESUPUESTOS_MOCK.filter(p => p.estado === "por_enviar" || p.estado === "borrador")
    : PRESUPUESTOS_MOCK.filter(p => p.estado === "esperando" || p.estado === "aceptado");

  if (formulario) {
    return (
      <div style={{ background: "var(--tp-crema)", minHeight: "100vh", padding: "16px 16px 90px" }}>
        <Formulario
          solicitud={formulario}
          navigate={navigate}
          onVolver={() => { if (desdeParam === "home-solucionador") navigate("/home-solucionador"); else setFormulario(null); }}
  ...
  return (
    <div style={{ background: "var(--tp-crema)", minHeight: "100vh", fontFamily: "var(--fuente)" }}>
      ...
      <header style={{ position: "sticky", top: 0, zIndex: 100, background: "var(--tp-crema)", borderBottom: "1px solid rgba(61,31,31,0.08)", padding: "14px 16px", display: "flex", alignItems: "center", gap: 12 }}>
        <button onClick={() => desdeParam === "home-solucionador" ? navigate("/home-solucionador") : navigate(-1)} style={{ border: "none", background: "none", cursor: "pointer", display: "flex" }}>
          <IconoVolver size={20} />
        </button>
        <h1 style={{ fontSize: 16, fontWeight: 800, color: "var(--tp-marron)", margin: 0, flex: 1 }}>Presupuestos</h1>
        <button type="button" onClick={() => setFormulario({ cliente: "", servicio: "", inicial: "C", color: "#B84030", monto: null })}
          style={{ width: 34, height: 34, borderRadius: "50%", background: "var(--tp-marron)", color: "var(--tp-crema)", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
          <Plus size={18} />
        </button>
      </header>

      {/* Filtros */}
      <div style={{ display: "flex", gap: 8, padding: "12px 16px", borderBottom: "1px solid rgba(61,31,31,0.06)" }}>
        {[
          ...
          <button key={f.id} type="button" onClick={() => setFiltro(f.id)}
            style={{ padding: "6px 14px", borderRadius: "var(--r-full)", cursor: "pointer", fontFamily: "var(--fuente)", fontSize: 12, fontWeight: 600, border: "none",
              background: filtro === f.id ? "var(--tp-marron)" : "rgba(61,31,31,0.06)",
              color: filtro === f.id ? "var(--tp-crema)" : "var(--tp-marron-suave)",
            }}>
            {f.label}
          </button>
        ))}
          ...
          return (
            <div key={p.id} style={{ background: "var(--tp-crema-clara)", borderRadius: "var(--r-lg)", padding: 14, border: "1px solid rgba(61,31,31,0.08)" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                <div style={{ width: 40, height: 40, borderRadius: "50%", background: p.color, color: "white", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15, fontWeight: 800, flexShrink: 0 }}>
                  {p.inicial}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ fontSize: 14, fontWeight: 700, color: "var(--tp-marron)", margin: "0 0 2px" }}>{p.cliente}</p>
                  <p style={{ fontSize: 12, color: "var(--tp-marron-suave)", margin: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{p.servicio}</p>
                </div>
                <span style={{ fontSize: 11, fontWeight: 700, padding: "3px 9px", borderRadius: "var(--r-full)", background: est.bg, color: est.color, display: "flex", alignItems: "center", gap: 3, flexShrink: 0 }}>
                  {est.icono} {est.label}
                </span>
              </div>
              ...
              {/* Monto + etapas */}
              {p.monto && (
                <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 10px", borderRadius: 10, background: "rgba(61,31,31,0.04)", marginBottom: 10 }}>
                  <p style={{ fontSize: 18, fontWeight: 900, color: "var(--tp-marron)", margin: 0 }}>
                    ${p.monto.toLocaleString()}
                  </p>
                  <span style={{ width: 1, height: 18, background: "rgba(61,31,31,0.15)" }} />
                  <p style={{ fontSize: 12, color: "var(--tp-marron-suave)", margin: 0 }}>
              ...
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                <span style={{ fontSize: 11, color: "var(--tp-marron-suave)", flex: 1 }}>{p.fecha}</span>

                {/* Chat siempre disponible */}
                <button type="button"
                  onClick={() => navigate(`/chat-s?usuarioId=1&nombre=${encodeURIComponent(p.cliente)}&inicial=${p.inicial}`)}
                  style={{ width: 32, height: 32, borderRadius: "50%", border: "1px solid rgba(61,31,31,0.15)", background: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: "var(--tp-marron-suave)" }}>
                  <MessageCircle size={14} />
                </button>

                {(p.estado === "por_enviar" || p.estado === "borrador") && (
                  <button type="button" onClick={() => setFormulario(p)}
                    style={{ padding: "7px 14px", borderRadius: "var(--r-full)", background: "var(--tp-rojo)", color: "var(--tp-crema)", border: "none", cursor: "pointer", fontFamily: "var(--fuente)", fontSize: 12, fontWeight: 700, display: "flex", alignItems: "center", gap: 5 }}>
                ...
                {p.estado === "esperando" && (
                  <button type="button" onClick={() => setFormulario(p)}
                    style={{ padding: "7px 14px", borderRadius: "var(--r-full)", background: "rgba(61,31,31,0.08)", color: "var(--tp-marron)", border: "none", cursor: "pointer", fontFamily: "var(--fuente)", fontSize: 12, fontWeight: 600 }}>
                    Ver
                  </button>
                )}

                {p.estado === "aceptado" && (
                  <span style={{ padding: "7px 12px", borderRadius: "var(--r-full)", background: "var(--verde-suave)", color: "var(--verde)", fontSize: 12, fontWeight: 700 }}>
                    ✓ Acordado
                  </span>
                )}
              </div>
